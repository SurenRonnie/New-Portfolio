...
export const TravelingImage = () => {
  const [targets, setTargets] = useState({ hero: null, about: null });
  ...
  const y = useSpring(useTransform([absY, scrollY], ([latestAbsY, latestScrollY]) => latestAbsY - latestScrollY), springConfig);
  ...
  const combinedX = useTransform([x, mouseParallaxX], ([latestX, latestMouseX]) => latestX + latestMouseX);
  const combinedY = useTransform([y, mouseParallaxY], ([latestY, latestMouseY]) => latestY + latestMouseY);
